class GroupAssignmentInvitation < ActiveRecord::Base
  ...
  def redeem_for(invitee, selected_group = nil, new_group_title = nil)
    ...
    invitees_group = find_or_create_group(repo_access, selected_group, new_group_title)

    invitees_group.repo_accesses << repo_access unless invitees_group.repo_accesses.include?(repo_access)

    find_or_create_group_assignment_repo(invitees_group)
  end
  ...
  def find_or_create_group(repo_access, selected_group, selected_group_title)
    ...
    Builders::GroupBuilder.create(title: selected_group_title, grouping: grouping)
  end

  def find_or_create_group_assignment_repo(invitees_group)
    ...
    Builders::GroupAssignmentRepoBuilder.build(group_assignment_params)
  end
end
